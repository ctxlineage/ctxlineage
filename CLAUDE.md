...
**Current state: v0.2.x (0.2.0 on PyPI; 0.2.1 is a maintenance release).** M1–M4 shipped: capture (openai + anthropic, sync/async/streaming), the four-view report, the span/tag lineage pipeline, the MCP server, redaction, and runnable examples. On top of that: `ctxlineage test` — deterministic context contracts, `_contract/` (#14) — and `ctxlineage import --from claude-code`, `_import/` (#57), plus a pytest plugin (`pytest --ctxlineage`). All four assertion classes named in `docs/vision/context-contract-testing.md` §8 now have a rule: structural (`window_budget`, `requires_segment`), regression/differential (`segment_diff`), metamorphic (`metamorphic`), provenance (`grounded`). Two of them read a **second recorded run** named by the config; both `metamorphic` and `grounded` gate only where a `tag()` made the evidence exact. Build/test: `uv sync --all-extras`, `uv run pytest`, `uv run ruff check . && uv run ruff format .`.
